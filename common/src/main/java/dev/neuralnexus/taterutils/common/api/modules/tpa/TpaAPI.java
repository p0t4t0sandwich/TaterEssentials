package dev.neuralnexus.taterutils.common.api.modules.tpa;
...
public class TpaAPI {
    ...
    public void checkExpired() {
        data.getRequests().stream().filter(TPRequest::isExpired).forEach(request -> {
    ...
    public static class TPRequest {
        ...
        public void sendExpireMessage() {
            sender.sendMessage(Utils.substituteSectionSign("&aYour teleport request to &e" + receiver.getName() + " &ahas expired."));
        }
    ...
    private static class Data {
         ...
         */
        public Optional<TPRequest> getRequest(Player sender) {
            return requests.stream().filter(request -> request.getSender().equals(sender)).findFirst();
        }
